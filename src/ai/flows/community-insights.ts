...
export async function getCommunityInsights(input: CommunityInsightsInput): Promise<CommunityInsightsOutput> {
  const CommunityInsightsInputSchema = z.object({
    location: z.string().describe('The user\'s location to find nearby community data (e.g., district, state).'),
  });
  
  const CommunityInsightsOutputSchema = z.object({
    commonCrops: z.array(z.object({
        name: z.string().describe('The name of the commonly grown crop.'),
        notes: z.string().describe('Brief notes about this crop in the region (e.g., "High yield, good market price").'),
    })).describe('A list of common crops grown in the area.'),
    commonDiseases: z.array(z.object({
        name: z.string().describe('The name of the common disease.'),
        crop: z.string().describe('The primary crop this disease affects.'),
        prevalence: z.string().describe('How common the disease is (e.g., "High", "Moderate").'),
    })).describe('A list of common diseases reported in the area.'),
    marketTrends: z.array(z.object({
        crop: z.string().describe('The crop name.'),
        trend: z.string().describe('The market trend (e.g., "Price increasing", "Stable demand").'),
        reason: z.string().describe('A brief reason for the trend.'),
    })).describe('A list of current market trends.'),
    effectiveTreatments: z.array(z.object({
        disease: z.string().describe('The disease name.'),
        treatment: z.string().describe('The effective treatment method.'),
        effectiveness: z.string().describe('How effective the treatment is reported to be (e.g., "Very effective", "Moderately effective").'),
    })).describe('A list of effective treatments reported by the community.'),
  });
...
The data should look realistic for the region. Do not use real names or specific farm data.

**Location:** {{{location}}}

**Task:**
Generate a JSON object containing insights for the four categories:
1.  **Common Crops:** List 3-4 crops commonly grown in the area with brief notes.
2.  **Common Diseases:** List 2-3 prevalent diseases, the crops they affect, and their prevalence.
3.  **Market Trends:** List 2-3 current market trends for local crops.
4.  **Effective Treatments:** List 2-3 treatments that have been reported as effective for common local diseases.
...
export type CommunityInsightsInput = z.infer<ReturnType<typeof getCommunityInsights>['inputSchema']>;
export type CommunityInsightsOutput = z.infer<ReturnType<typeof getCommunityInsights>['outputSchema']>;
