...
import { getCommunityInsights, type CommunityInsightsInput, type CommunityInsightsOutput } from "@/ai/flows/community-insights";
import { z } from "zod";
...
const CommunityInsightsSchema = z.object({
  location: z.string().min(3, "Location is required and must be at least 3 characters."),
});
...
export async function generateCommunityInsights(
  ...
  try {
    ...
    return {
      form: validatedFields.data,
      error: e.message || "An unexpected error occurred while fetching community insights.",
    };
  }
}
